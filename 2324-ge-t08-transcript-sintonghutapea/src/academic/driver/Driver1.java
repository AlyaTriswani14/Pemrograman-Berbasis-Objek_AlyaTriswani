 ...
 public class Driver1 {
    ...
    public static void main(String[] _args) {
                ...
                if (command.equals("lecturer-add")) {
                    ...
                    for (CourseOpening co : courseOpenings) {
                        if (co.getCourseCode().equals(courseCode) &&
                            co.getAcademicYear().equals(academicYear) &&
                            co.getSemester().equals(semester)) {
                            courseOpening = co;
                            break;
                        }
                    }
                    if (courseOpening != null) {
                        Enrollment enrollment = new Enrollment(courseOpening, studentId);
                    ...
                    if (!idCourseCodeExist) {
                        Course course = new Course(courseCode, courseName, credits, passingGrade);
                        courses.add(course);
                    }
                }
                    ...
                    String lecturerInitialList = tokens[4];
                    ...
                    if (course != null) {
                        ...
                        CourseOpening courseOpening = new CourseOpening(course, academicYear, semester, courseLecturers);
                    ...
                    String studentId = tokens[1];
                    printStudentTranscript(studentId, enrollments, courses, students);
                }
            }
        }
        ...
        if (isFinished) {
            ...
            for(Course course : courses){
                System.out.println(course.getCourseCode() + "|" + course.getCourseName() + "|" + course.getCredits() + "|" + course.getPassingGrade());
            }
    ...
    private static void printStudentTranscript(String studentId, List<Enrollment> enrollments, List<Course> courses, List<Student> students) {
        Student student = null;
        for (Student s : students) {
            if (s.getId().equals(studentId)) {
                student = s;
        ...
        for (Enrollment e : enrollments) {
            ...
            String key = e.getCourseCode();
            if (!latestEnrollments.containsKey(key) || isNewer(e, latestEnrollments.get(key))) {
                latestEnrollments.put(key, e);
            }
        }
    
        List<Enrollment> filtered = new ArrayList<>(latestEnrollments.values());
        
        filtered.sort((e1, e2) -> {
            int yearCompare = e1.getAcademicYear().compareTo(e2.getAcademicYear());
            if (yearCompare != 0) return yearCompare;
            return e1.getSemester().equals("odd") ? -1 : 1;
        });
    
        filtered.sort((e1, e2) -> e1.getCourseCode().compareTo(e2.getCourseCode()));
        ...
        for (Enrollment e : filtered) {
            ...
            if (course == null || e.getGrade().equals("None")) continue;
    
            int credit = Integer.parseInt(course.getCredits());
        ...
        System.out.printf("%s|%s|%s|%s|%.2f|%d\n", student.getId(), student.getName(), student.getYear(), student.getMajor(), gpa, totalCredits);
    
        for (Enrollment e : filtered) {
            String displayGrade = e.getGrade();
            if (!e.getPreviousGrade().equals("None")) {
                displayGrade += "(" + e.getPreviousGrade() + ")";
            }
            System.out.printf("%s|%s|%s|%s|%s\n", e.getCourseCode(), e.getStudentId(), e.getAcademicYear(), e.getSemester(), displayGrade);
        }
    }
    
    private static boolean isNewer(Enrollment e1, Enrollment e2) {
        int yearCompare = e1.getAcademicYear().compareTo(e2.getAcademicYear());
        if (yearCompare != 0) return yearCompare > 0;
        return e1.getSemester().equals("even") && e2.getSemester().equals("odd");
    }
