...
import academic.model.Individu;
    ...
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            if (input.startsWith("student-add#")) {
                ...
                Student.addStudent(em, tokens[1], tokens[2], tokens[3], tokens[4]);
            } else if (input.equals("student-show-all")) {
                Student.showAllStudents(em);
            } else if (input.startsWith("course-add#")) {
                String[] tokens = input.split("#");
                Course.addCourse(em, tokens[1], tokens[2], Integer.parseInt(tokens[3]), tokens[4]);
            ...
            } else if (input.startsWith("student-transcript#")) {
                String studentId = input.split("#")[1];
                Student.showTranscript(em, studentId);
            }
        }
        em.close();
        emf.close();
